...
14. **M3 slice 11 local implementation is now in place**: `Program.cs` now honors forwarded public scheme/host before guarded same-origin enforcement, `SolverApiTests.cs` locks matching/mismatched forwarded-origin behavior, and `HostRoutingIntegrationTests.cs` locks rollback shell behavior under forwarded public authority.
15. The remaining M3 slice 11 gate is **deployment smoke**: record one real public-origin guarded solve plus one rollback confirmation before marking the slice complete or naming slice 12.
16. `SolverService/SatisfactoryTools.Solver.Api.Tests/Fixtures/Planner/*.json` is now the authoritative captured artifact set for F001-F008.
