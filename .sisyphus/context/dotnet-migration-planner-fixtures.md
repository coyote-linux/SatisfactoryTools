...
13. The next M3 slice is **10**: review guarded default-on rollout behind the hardened internal route while preserving the one-flag rollback path.
14. `SolverService/SatisfactoryTools.Solver.Api.Tests/Fixtures/Planner/*.json` is now the authoritative captured artifact set for F001-F008.
