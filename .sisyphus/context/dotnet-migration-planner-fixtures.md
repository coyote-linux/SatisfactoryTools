...
14. The next M3 slice is **11**: validate the default-on guarded planner path under deployed public-host topology and confirm rollback smoke without widening `/v2/*` or starting UI cutover work.
15. `SolverService/SatisfactoryTools.Solver.Api.Tests/Fixtures/Planner/*.json` is now the authoritative captured artifact set for F001-F008.
