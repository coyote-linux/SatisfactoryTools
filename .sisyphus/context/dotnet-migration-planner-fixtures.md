...
12. **M3 slice 9 is now complete**: `SolverApiTests.cs` and `InternalPlannerAccessPolicyTests.cs` lock same-origin access hardening for `/_internal/planner/calculate`, while the guarded planner path stays default-off and the slice 8b browser regressions remain green.
13. **M3 slice 10 is now complete**: the unified shell now defaults guarded planner calculation on, `PlannerBrowserRegressionTests.cs` exercises that real default, and explicit false rollback still routes F004-style planner solves through `/v2/solver`.
14. **M3 slice 11 is now complete**: `Program.cs` honors forwarded public scheme/host before guarded same-origin enforcement, `SolverApiTests.cs` and `HostRoutingIntegrationTests.cs` lock the topology behavior locally, and the deployed fork at `https://ficsit.spugnort.com/` has now passed both the default-on guarded smoke and explicit rollback smoke.
15. **M4 slice 1 is now complete**: the host reserves `/beta/*` away from legacy shell fallback, `Program.cs` exposes a flag-gated placeholder at `/beta/production`, and route/integration tests lock that seam without touching Angular planner ownership.
16. The next slice should stay in **M4**: replace the placeholder behind `/beta/production` with the first isolated beta planner shell or layout while keeping `/{version}/production` Angular-owned.
17. `SolverService/SatisfactoryTools.Solver.Api.Tests/Fixtures/Planner/*.json` is now the authoritative captured artifact set for F001-F008; M4 slice 1 did not require new fixture expectations yet.
